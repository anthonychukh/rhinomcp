...
internal sealed class BridgeOperation
{
    ...
    public BridgeOperation(string command, string requestId)
    {
    ...
    public JObject ToJson(bool includeResult)
    {
        lock (sync)
        {
            ...
            JObject modal = null;
            if (state is "running" or "cancel_requested")
            {
...
internal static class BridgeOperationRegistry
{
    ...
    public static (BridgeOperation Operation, bool Created) GetOrCreate(string command, string requestId)
    {
        lock (RegistrationLock)
        {
            ...
            {
                if (!string.Equals(existing.Command, command, StringComparison.Ordinal))
                {
                    throw new InvalidOperationException(
                        $"execution.request_id '{requestId}' is already associated with command '{existing.Command}'.");
                }
                return (existing, false);
            }

            var operation = new BridgeOperation(command, requestId);
    ...
    public static JObject TryGetModalWindow()
    {
        ...
        try
        {
            EnumWindows((hWnd, _) =>
            {
                ...
                if (!standardDialog && IsWindowEnabled(owner)) return true;
